...
async def main():
    ...
    alice_balance = await rest_client.account_balance(account_alice.address())
    bob_balance = await rest_client.account_balance(account_bob.address())
    ...
    bob_amount = 100_000_000  # Bob starts with 0 APT
    
    await faucet_client.fund_account(account_alice.address(), alice_amount)
    print(f"Account funded Alice({alice_amount}) successfully")

    await faucet_client.fund_account(account_bob.address(), bob_amount)
    print(f"Account funded Bob({bob_amount}) successfully")
